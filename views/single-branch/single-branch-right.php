...
<div id="tabsWithStyle" class="tab_area">
    ...
    <?php if ( $fand_active_tab == 'products' ) : ?>
        <div class="" id="products">
            <div class="product_area">
                <div id="products-wrapper" class="products-wrapper">
                    ...
                    if ( $fand_products->have_posts() ) :
                        ...
                        $fand_paged = $fand_products->query_vars['paged']; 
                        ?>
            ...
            <?php do_action( 'wcfmmp_store_after_products', $fand_vendor_id ); ?>
        </div>
                ...
                <div class="recent_reviews">
                    <div class="bd_review_section">
                        <?php if ( ! empty( $fand_data['reviews'] ) ) : ?>
                            <?php foreach ( $fand_data['reviews'] as $fand_review ) : ?>
                                <div class="review_section">
                                    <div class="lft user_photo">
                                        ...
                                        <div class="rated">
                                            ...
                                            <div class="user_rated"><?php echo number_format($fand_review['rating'], 1); ?></div>
                                        </div>
                                    </div>

                                    <div class="rgt user_review_sec">
                                        <div class="user_review_sec_left" style="display: inline-block; float: left; width: 60%;">
                                            <div class="user_name"><?php echo esc_html( $fand_review['comment_author'] ); ?></div>
                                            <div class="user_review_area">
                                                <span class="user_date"><?php echo esc_html( date_i18n( 'j F Y G\hi', strtotime( $fand_review['comment_date'] ) ) ); ?></span>
                                            </div>
                                            <div class="user_review_text">
                                                <p><?php echo nl2br( esc_html( $fand_review['comment_content'] ) ); ?></p>
                                            </div>
                                        </div>

                                        <div class="bd_rating_area" style="float: right; width: 35%;">
                                            <?php if ( !empty($fand_review['sub_ratings']) ) : ?>
                                                <?php foreach ( $fand_review['sub_ratings'] as $fand_sub ) : 
                                                    $fand_val = floatval($fand_sub['value']);
                                                ?>
                                                    <div class="rating_box">
                                                        <?php for ( $fand_i = 1; $fand_i <= 5; $fand_i++ ) : ?>
                                                            <i class="wcfmfa fa-star <?php echo ( $fand_i <= $fand_val ) ? 'selected' : ''; ?>" aria-hidden="true" style="color: <?php echo ( $fand_i <= $fand_val ) ? '#ffb600' : '#ccc'; ?>;"></i>
                                                        <?php endfor; ?>
                                                        <span><?php echo number_format($fand_val, 1); ?>&nbsp;<?php echo esc_html($fand_sub['key']); ?></span>
                                                    </div>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                        </div>
